...
namespace TrakHound.Server.Plugins.Shifts
{
    public class ShiftTable : IServerPlugin
    {
        ...
        public void Update_CurrentShiftInfo(ReturnData returnData)
        {
            ...
            var infos = new List<TrakHound.Databases.Tables.Variables.VariableData>();

            // Update shift_current in "Variables" table
            var shiftInfo = CurrentShiftInfo.Get(configuration, returnData.Header.CreationTime);
            if (shiftInfo != null)
            {
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_name", shiftInfo.name, timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_id", shiftInfo.id, timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_type", shiftInfo.type, timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_date", shiftInfo.date, timestamp));

                // Local
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_begintime", shiftInfo.shiftStart.ToString(), timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_endtime", shiftInfo.shiftEnd.ToString(), timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_currenttime", shiftInfo.currentTime.ToString(), timestamp));

                // UTC
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_begintime_utc", shiftInfo.shiftStartUTC.ToString(), timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_endtime_utc", shiftInfo.shiftEndUTC.ToString(), timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_currenttime_utc", shiftInfo.currentTimeUTC.ToString(), timestamp));
            }
            else
            {
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_name", "", timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_id", "", timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_type", "", timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_date", "", timestamp));

                // Local
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_begintime", "", timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_endtime", "", timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_currenttime", "", timestamp));

                // UTC
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_begintime_utc", "", timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_endtime_utc", "", timestamp));
                infos.Add(new TrakHound.Databases.Tables.Variables.VariableData("shift_currenttime_utc", "", timestamp));
            }

            TrakHound.Databases.Tables.Variables.Update(configuration.Databases_Server, infos.ToArray(), tablePrefix);
        ...
        {
            var sc = ShiftConfiguration.Get(configuration);
            if (sc != null)
            {
        ...
        List<ShiftRowInfo> GetExistingValues()
        {
            ...
            DataTable dt = Table.Get(configuration.Databases_Server, GetTableName(TableNames.Shifts), filter);
            if (dt != null)
            {
        ...
        public void UpdateShiftRows(List<ShiftRowInfo> infos)
        {
            ...
            foreach (var info in infos)
            {
                ...
                values.Add(info.Start.ToString("yyyy-MM-dd HH:mm:ss"));
                values.Add(info.End.ToString("yyyy-MM-dd HH:mm:ss"));
                values.Add(info.StartUtc.ToString("yyyy-MM-dd HH:mm:ss"));
                values.Add(info.EndUtc.ToString("yyyy-MM-dd HH:mm:ss"));
            ...
            foreach (ShiftRowInfo newInfo in newInfos)
            {
                if (rowInfos != null)
                {
                    ...
                    if (currentShiftInfo != null)
                    {
                        ...
                        RemoveOldRowInfos(rowInfos, currentShiftInfo);
                    }
        ...
        void RemoveOldRowInfos(List<ShiftRowInfo> infos, CurrentShiftInfo shiftInfo)
        {
            ...
            infos.RemoveAll(x => x.End < start.Subtract(TimeSpan.FromDays(2)));
        }
