...
namespace TrakHound.Server.Plugins.OEE.Database
{
    public static class CycleBased
    {
        ...
        public static OEEData GetWholeDay(DeviceConfiguration config, DateTime date)
        {
            ...
            var dt = Table.Get(config.Databases_Server, Global.GetTableName(TABLENAME, config.DatabaseId), "WHERE start >= '" + start.ToString("yyyy-MM-dd HH:mm:ss") + "' AND end <'" + end.ToString("yyyy-MM-dd HH:mm:ss") + "'");
            if (dt != null && dt.Rows.Count > 0)
            {
                return OEEData.FromCycleDataRow(dt.Rows[0]);
            }
