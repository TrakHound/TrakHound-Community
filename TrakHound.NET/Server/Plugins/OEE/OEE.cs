...
using TrakHound.Server.Plugins.Cycles;
...
namespace TrakHound.Server.Plugins.OEE
{
    public class OEE : IServerPlugin
    {
        ...
        public string Name { get { return "TrakHound.Server.Plugins.OEE"; } }
        ...
        private void ProcessCycles(List<CycleData> cycles)
        {
            ...
            foreach (var cycle in cycles)
            {
                if (cycleOeeData == null)
                {
                    ...
                    if (cycleOeeData == null)
                    {
                        ...
                        cycleOeeData.ConstantQuality = 1; // Change when Quality (TrakHound.Server.Plugins.Parts) is implemented
                ...
                if (oeeData == null)
                {
                    ...
                    {
                        oeeData = new OEEData();
                        oeeData.ConstantQuality = 1; // Change when Quality (TrakHound.Server.Plugins.Parts) is implemented
            ...
            var dayDatas = Database.CycleBased.GetWholeDay(config, DateTime.Now);
            if (dayDatas != null)
            {
                SendDayOee(dayDatas.Oee);
                SendDayAvailability(dayDatas.Availability);
                SendDayPerformance(dayDatas.Performance);
            }
        }
            ...
            foreach (var cycle in cycles)
            {
                if (shiftOeeData == null)
                {
                    ...
                    if (shiftOeeData == null)
                    {
                        ...
                        shiftOeeData.ConstantQuality = 1; // Change when Quality (TrakHound.Server.Plugins.Parts) is implemented
        ...
        private static string GetUnderscoreDelimitedValue(string val)
        {
            ...
            if (words != null && words.Length > 0)
            {
                ...
                {
                    builder.Append(TrakHound.Tools.String_Functions.UppercaseFirst(words[i]));
            ...
            return TrakHound.Tools.String_Functions.UppercaseFirst(val);
        }
            ...
            foreach (var cycle in cycles)
            {
                if (segmentOeeData == null)
                {
                    ...
                    if (segmentOeeData == null)
                    {
                        ...
                        segmentOeeData.ConstantQuality = 1; // Change when Quality (TrakHound.Server.Plugins.Parts) is implemented
        ...
        void SendDayOee(double val)
        {
            var edata = new EventData();
            edata.Id = "oee_day_oee";
            edata.Data01 = config;
            edata.Data02 = val;
            if (SendData != null) SendData(edata);
        }

        void SendDayAvailability(double val)
        {
            var edata = new EventData();
            edata.Id = "oee_day_availability";
            edata.Data01 = config;
            edata.Data02 = val;
            if (SendData != null) SendData(edata);
        }

        void SendDayPerformance(double val)
        {
            var edata = new EventData();
            edata.Id = "oee_day_performance";
            edata.Data01 = config;
            edata.Data02 = val;
            if (SendData != null) SendData(edata);
        }
