...
namespace TrakHound_Server.Plugins.OEE.Database
{
    public static class CycleBased
    {
        ...
        public const string TABLENAME = Names.OEE_Cycles;
        ...
        {
            var dt = Table.Get(DataManagement.Database.Configuration, Table.GetName(TABLENAME, config.DatabaseId), "WHERE shift_id='" + shiftId + "' AND cycle_id='" + cycleId + "' AND cycle_instance_id='" + cycleInstanceId + "'");
        ...
        public static OEEData GetWholeDay(DeviceConfiguration config, DateTime date)
        {
            var start = new DateTime(date.Year, date.Month, date.Day, 0, 0, 0);
            var end = start.AddDays(1);

            var dt = Table.Get(DataManagement.Database.Configuration, Table.GetName(TABLENAME, config.DatabaseId), "WHERE start >= '" + start.ToString("yyyy-MM-dd HH:mm:ss") + "' AND end <'" + end.ToString("yyyy-MM-dd HH:mm:ss") + "'");
            if (dt != null && dt.Rows.Count > 0)
            {
                return OEEData.FromCycleDataRow(dt.Rows[0]);
            }
