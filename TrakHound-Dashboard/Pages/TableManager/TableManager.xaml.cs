...
namespace TrakHound_Dashboard.Pages.TableManager
{
    ...
    public partial class Plugin : UserControl
    {
        ...
        void LoadTableList_Worker(object o)
        {
            ...
            string[] tableNames = Table.List(config.Databases_Client);
        ...
        void lb_Device_Selected(ListButton lb)
        {
            foreach (ListButton olb in DeviceList.OfType<ListButton>()) if (olb != lb) olb.IsSelected = false;
            lb.IsSelected = true;
        ...
        void LoadInfo_Worker(object loadTableParameters)
        {
            ...
            long rowCount = Table.GetRowCount(ltp.Config.Databases_Client, ltp.Tablename);

            long tablesize = Table.GetSize(ltp.Config.Databases_Client, ltp.Tablename);
        ...
        private void TableInfoRefresh_Button_Clicked(TrakHound_UI.Button bt)
        {
        ...
        void LoadTable_Worker(object loadTableParameters)
        {
            ...
            // Row Limit
            int limit = rowLimits[Properties.Settings.Default.TableManagerRowDisplayIndex];
            ...
            // Get MySQL table
            DataTable dt = Table.Get(ltp.Config.Databases_Client, ltp.Tablename, limit, offset);
        ...
        private void DropTables_Button_Clicked(TrakHound_UI.Button bt)
        {
            var result = TrakHound_UI.MessageBox.Show("Delete Selected Tables?", "Drop Tables", MessageBoxButtons.YesNo);
            if (result == MessageBoxDialogResult.Yes)
            {
                string[] tablenames = SelectedTables.Select(x => x.Text).Distinct().ToArray();

                Table.Drop(SelectedDevice.Databases_Client, tablenames);
        ...
        {
            int rowLimit = rowLimits[Properties.Settings.Default.TableManagerRowDisplayIndex];
        ...
        void PageButton_Clicked(TrakHound_UI.Button bt)
        {
        ...
        {
            int index = Properties.Settings.Default.TableManagerRowDisplayIndex;
        ...
        {
            Properties.Settings.Default.TableManagerRowDisplayIndex = limit;
