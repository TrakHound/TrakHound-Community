    ...
    public partial class StatusData
    {
        ...
        private static EventData GetShifts(ShiftData shiftData, DeviceConfiguration config)
        {
            ...
            if (shiftData.shiftDate != null && shiftData.shiftName != null)
            {
                ...
                var dt = Table.Get(config.Databases_Client, Global.GetTableName(TableNames.Shifts, config.DatabaseId), "WHERE Start >= '" + start.ToString("yyyy-MM-dd HH:mm:ss") + "' AND End < '" + end.ToString("yyyy-MM-dd HH:mm:ss") + "'");
                if (dt != null)
                {
                    var data = new EventData();
